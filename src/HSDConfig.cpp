...
bool HSDConfig::readMainConfigFile()
{
  ...
  if(m_mainConfigFile.read(fileBuffer, MAX_SIZE_MAIN_CONFIG_FILE))
  {
    ...
    if (json.success()) 
    {
      ...
      if(json.containsKey(JSON_KEY_HOST) && json.containsKey(JSON_KEY_WIFI_SSID) && json.containsKey(JSON_KEY_WIFI_HOSTNAME) && json.containsKey(JSON_KEY_WIFI_PSK) && 
         json.containsKey(JSON_KEY_MQTT_SERVER) && json.containsKey(JSON_KEY_MQTT_USER) && json.containsKey(JSON_KEY_MQTT_PASS) && json.containsKey(JSON_KEY_MQTT_STATUS_TOPIC) && json.containsKey(JSON_KEY_MQTT_TEST_TOPIC) && json.containsKey(JSON_KEY_MQTT_WILL_TOPIC) &&
         json.containsKey(JSON_KEY_LED_COUNT))
      {
        ...
        setWifiHostname(json[JSON_KEY_WIFI_HOSTNAME]);
        setWifiPSK(json[JSON_KEY_WIFI_PSK]);
        setMqttServer(json[JSON_KEY_MQTT_SERVER]);
        setMqttServerUser(json[JSON_KEY_MQTT_USER]);
        setMqttServerPass(json[JSON_KEY_MQTT_PASS]);
...
void HSDConfig::printMainConfigFile(JsonObject& json)
{
  ...
  Serial.print  (F("  • Hostname        : ")); Serial.println((const char*)(json[JSON_KEY_WIFI_HOSTNAME]));
  Serial.println(F("  • wifiPSK         : not shown"));
  Serial.print  (F("  • mqttServer      : ")); Serial.println((const char*)(json[JSON_KEY_MQTT_SERVER]));
  Serial.print  (F("  • mqttServerUser  : ")); Serial.println((const char*)(json[JSON_KEY_MQTT_USER]));
...
void HSDConfig::writeMainConfigFile()
{
  ...
  json[JSON_KEY_WIFI_HOSTNAME] = m_cfgWifiHostname;
  json[JSON_KEY_WIFI_PSK] = m_cfgWifiPSK;
  json[JSON_KEY_MQTT_SERVER] = m_cfgMqttServer;
  json[JSON_KEY_MQTT_USER] = m_cfgMqttServerUser;
  json[JSON_KEY_MQTT_PASS] = m_cfgMqttServerPass;
...
const char* HSDConfig::getWifiHostname() const
{
  return m_cfgWifiHostname;
}

bool HSDConfig::setWifiHostname(const char* wifihost)
{
  strncpy(m_cfgWifiHostname, wifihost, MAX_WIFI_HOSTNAME_LEN);
  m_cfgWifiHostname[MAX_WIFI_HOSTNAME_LEN] = '\0';
  return true;
}
...
const char* HSDConfig::getMqttServerUser() const
{
  return m_cfgMqttServerUser;
}

bool HSDConfig::setMqttServerUser(const char* mqttuser)
{
  strncpy(m_cfgMqttServerUser, mqttuser, MAX_MQTT_SERVER_USER_LEN);
  m_cfgMqttServerUser[MAX_MQTT_SERVER_USER_LEN] = '\0';
  return true;
}

const char* HSDConfig::getMqttServerPass() const
{
  return m_cfgMqttServerPass;
}

bool HSDConfig::setMqttServerPass(const char* mqttpass)
{
  strncpy(m_cfgMqttServerPass, mqttpass, MAX_MQTT_SERVER_PASS_LEN);
  m_cfgMqttServerPass[MAX_MQTT_SERVER_PASS_LEN] = '\0';
  return true;
}
