...
bool HSDMqtt::reconnect()
{
  ...
  // Username && Pass
  const char* mqttUser = m_config.getMqttServerUser();
  const char* mqttPass = m_config.getMqttServerPass();
  ...
  // if username AND password not empty, connect with credentials
  if(strlen(mqttUser) != 0 && strlen(mqttPass) != 0){
    ...
    if(isTopicValid(willTopic))
      {
        connected = m_pubSubClient.connect(clientId.c_str(), mqttUser, mqttPass, willTopic, 0, true, "off");
      }
      else
      {
        connected = m_pubSubClient.connect(clientId.c_str(), mqttUser, mqttPass);
      }
